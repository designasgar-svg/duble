...
def translate_safe_batch(text_list, target_lang):
    """ترجمه دسته‌ای با تضمین عدم بازگرداندن None"""
    if not target_lang or target_lang == "none" or not text_list:
        return text_list

    try:
        translated_list = GoogleTranslator(source='auto', target=target_lang).translate_batch(text_list)
        clean_results = []
        for orig, trans in zip(text_list, translated_list):
            if trans and isinstance(trans, str) and trans.strip():
                clean_results.append(trans.strip())
            else:
                clean_results.append(orig.strip())
        return clean_results
    except Exception as e:
        print(f"[!] خطای شبکه در ترجمه دسته‌ای: {e}")
        return [str(t) for t in text_list]
...
def process_single_sub(sub_info, gender, audio_lang):
    ...
    # تولید صدا فقط در صورت وجود متن غیرخالی
    ...
    try:
        ...
        def run_background_process():
            try:
                ...
                chunk_size = 30  # دسته‌های ۳۰ تایی برای ترجمه سریع و ایمن
                ...
                for chunk_start in range(0, total_subs, chunk_size):
                    ...
                    display_texts = translate_safe_batch(chunk_orig_texts, text_lang)
                    if audio_lang == text_lang:
                        audio_texts = display_texts
                    else:
                        audio_texts = translate_safe_batch(chunk_orig_texts, audio_lang)
