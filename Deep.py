...
def batch_translate(texts, target_lang):
    """ترجمه گروهی و امن برای جلوگیری از خطای Too Many Requests"""
    if not target_lang or target_lang == "none" or not texts:
        return texts
    
    translated_results = []
    chunk_size = 40  # ارسال دسته‌های ۴۰تایی برای ماندن در محدوده مجاز گوگل
    ...
    for i in range(0, len(texts), chunk_size):
        ...
        try:
            ...
            if res:
                translated_results.extend([r if r else t for r, t in zip(res, chunk)])
            else:
                translated_results.extend(chunk)
        except Exception as e:
            print(f"[!] خطای ترجمه گروهی: {e}")
            translated_results.extend(chunk)
        time.sleep(0.6)  # وقفه کوتاه بین دسته‌ها جهت پایداری
        ...
        def run_background_process():
            try:
                ...
                for i, sub in enumerate(subs):
                    ...
                    raw_sub_data.append((i, start_ms, end_ms))
                ...
                socketio.emit('status_update', {'msg': 'در حال ترجمه متن‌ها...'})
                
                # ترجمه گروهی به زبان متن و زبان صوت
                translated_text_lang = batch_translate(orig_texts, text_lang)
                if audio_lang == text_lang:
                    translated_audio_lang = translated_text_lang
                else:
                    translated_audio_lang = batch_translate(orig_texts, audio_lang)

                for idx, (i, start_ms, end_ms) in enumerate(raw_sub_data):
                    progress = min(100, int(((idx + 1) / total_subs) * 100))
                    socketio.emit('status_update', {'msg': f'در حال ساخت صوت ({progress}%)...'})

                    d_text = translated_text_lang[idx]
                    a_text = translated_audio_lang[idx]
