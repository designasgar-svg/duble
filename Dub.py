...
def translate_safe_chunk(text_list, target_lang):
    ...
    try:
        # ارسال مستقیم آرایه متون (گوگل در ۱ درخواست همه را ترجمه می‌کند)
        translations = translator.translate(text_list, dest=target_lang)
        return [t.text if (t and t.text) else orig for t, orig in zip(translations, text_list)]
    except Exception as e:
        print(f"[!] خطای شبکه در ترجمه این دسته: {e}")
        # در صورت قطعی اینترنت یا خطا، همان متن اصلی جایگزین می‌شود تا TTS ارور ندهد
        return text_list
    ...
    try:
        ...
        def run_background_process():
            try:
                ...
                # پردازش در دسته‌های ۵۰ تایی (در کل ۱۲ درخواست به گوگل فرستاده می‌شود!)
                chunk_size = 50
                ...
                for chunk_start in range(0, total_subs, chunk_size):
                    ...
                    # ۲. ساخت صدا و ارسال مستقیم
                    for idx, (i, orig_text, start_ms, end_ms) in enumerate(chunk_raw):
                        d_text = display_texts[idx]
                        a_text = audio_texts[idx]
